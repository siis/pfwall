 ...
 * t->p->user_stack.trace.ept_ind is -1 or not.
 *
...
void user_unwind(struct pf_packet_context *p)
{
	struct unw_t unw;
	...
	do { /* for each vm area */
		...
		if (eh_len == 0) {
			...
			if (t->p->user_stack.trace.nr_entries > 0 && IS_BIN_VMA(t, vma) &&
				...
				us_init(&(t->p->user_stack));
				static_save_stack_trace_user(&(t->p->user_stack.trace));
				vma_start_ino(t);
			}
			goto fail_put_stack_pages;
		}
...
int pft_interface_context(struct pf_packet_context *p)
{
	...
	p->user_stack.trace.max_entries	= MAX_NUM_FRAMES;
	...
	ret = user_interpreter_unwind(p); 
	
end:
	if (ret < 0 || p->user_stack.trace.ept_ind == -1) {
...
int pft_vm_area_name_context(struct pf_packet_context *p)
{
	...
	while (i < MAX_NUM_FRAMES - 1) {
		address = p->user_stack.trace.entries[i];
		...
		if (IS_ERR(ptemp)) {
			...
			ret = -EINVAL;
			strcpy(p->user_stack.trace.vm_area_strings[i], "d_pathfail");
		}
		strcpy(p->user_stack.trace.vm_area_strings[i], ptemp);
		i++;
	}
...
static int __init pft_entrypoint_context_init(void)
{
	int rc = 0;
	rc = pf_register_context(PF_CONTEXT_INTERFACE, &pft_interface_context);
	if (rc < 0) {
		printk(KERN_INFO PFWALL_PFX "Failed to register entrypoint module: %d\n", rc);
	}
	rc = pf_register_context(PF_CONTEXT_VM_AREA_STRINGS, &pft_vm_area_name_context);
	if (rc < 0) {
		printk(KERN_INFO PFWALL_PFX "Failed to register vm_area_name module: %d\n", rc);
	}
	return rc;
}
module_init(pft_entrypoint_context_init);
