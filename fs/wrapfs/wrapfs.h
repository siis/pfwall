...
/* 
 * Irrelevant features of unionfs
 * Copyup - No writes done on upper layer (except file creation by adversary,
 * 		which should not be permanent anyway).  As soon as any write
 * 		is attempted that may be permanent, we immediately switch to
 * 		lower branch. 
 * 
 * Branch updates - To keep code simple, and since we only anyway have two
 * 			branches, no support for dynamic updates once mounted. 
 *
 */

/* 
 * Needed but as-yet unimplemented
 *
 * Whiteouts - Needed for exist checks (simply introduce a whiteout if you
 * 		don't want the file to be there.  Is it meaningful to whiteout
 * 		a non-adversarial lower directory file, or create one?
 *
 * Opacity - When deleting a directory, whiteout is created.  However, a file
 * 		can be created by adversary in its place.  A file on upper
 * 		branch should override anything (be it a directory or another
 * 		file) on the lower branch.  Does this need modification?
 * 		Directories on upper branch can either be unified with lower,
 * 		or opaque.  
 *
 */

/*
 * Added/modified features to unionfs
 * 
 * Redirect ot higher/lower based on adversary model (and probably exist
 * state). 
 *
 * Opacity: Needed for whiteout + creation in upper. 
 */
...
struct wrapfs_file_info {
	...
	const struct vm_operations_struct *higher_vm_ops; 
