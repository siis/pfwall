...
/*
 * __advfs_check_{inode,dentry,file} perform exhaustive sanity checking on
 ...
 */
void __advfs_check_inode(const struct inode *inode,
...
void __advfs_check_dentry(const struct dentry *dentry,
			    const char *fname, const char *fxn, int line)
{
	int bindex;
...
check_inode:
	...
	if (unlikely(advfs_is_newer_lower(dentry) && !advfs_is_negative_lower(dentry))) {
		PRINT_CALLER(fname, fxn, line);
...
void __advfs_check_file(const struct file *file,
			  const char *fname, const char *fxn, int line)
{
	int bindex;
	...
	__advfs_check_dentry(dentry, fname, fxn, line);
}

void __advfs_check_nd(const struct nameidata *nd,
			const char *fname, const char *fxn, int line)
{
	struct file *file;
	int printed_caller = 0;
...
void advfs___show_branch_counts(const struct super_block *sb,
			  const char *file, const char *fxn, int line)
{
	...
	for (i = 0; i < sbmax(sb); i++) {
		if (likely(sb->s_root))
			mnt = ADVFS_D(sb->s_root)->lower_paths[i].mnt;
		else
			mnt = NULL;
		printk(KERN_CONT "%d:",
...
void advfs___show_inode_times(const struct inode *inode,
			const char *file, const char *fxn, int line)
{
	struct inode *lower_inode;
	int bindex;
...
void advfs___show_dinode_times(const struct dentry *dentry,
			const char *file, const char *fxn, int line)
{
...
void advfs___show_inode_counts(const struct inode *inode,
			const char *file, const char *fxn, int line)
{
	struct inode *lower_inode;
	int bindex;
