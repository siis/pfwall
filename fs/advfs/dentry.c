...
bool advfs_is_negative_lower(const struct dentry *dentry)
{
	int bindex;
...
/*
 * Purge and invalidate as many data pages of a advfs inode.  This is
 ...
 */
bool __advfs_d_revalidate(struct dentry *dentry, struct dentry *parent,
			    bool willwrite)
{
	...
	if (advfs_is_newer_lower(dentry)) {
		...
		if (IS_ROOT(dentry)) {
			 ...
			 */
			dgen = 0;
			atomic_set(&ADVFS_D(dentry)->generation, dgen);
		}
		if (!willwrite)
			purge_inode_data(dentry->d_inode);
	}

	sbgen = atomic_read(&ADVFS_SB(dentry->d_sb)->generation);
	...
	pdgen = atomic_read(&ADVFS_D(parent)->generation);
	...
	if (advfs_realloc_dentry_private_data(dentry) != 0) {
		valid = false;
		goto out;
	}

	result = advfs_lookup_full(dentry, parent, interpose_flag);
	if (result) {
		if (IS_ERR(result)) {
			valid = false;
			goto out;
		...
		/*
		 * current advfs_lookup_backend() doesn't return
		 * a valid dentry
		 */
		dput(dentry);
		dentry = result;
	...
	if (unlikely(positive && advfs_is_negative_lower(dentry))) {
	 ...
	 */
	if (pdgen == sbgen)
		atomic_set(&ADVFS_D(dentry)->generation, sbgen);
	goto out;
...
validate_lowers:
	...
	for (bindex = bstart; bindex <= bend; bindex++) {
		 ...
		 * fanout nature of advfs (cf. file/directory open-file
	...
	if (valid) {
		 ...
		 * purged all advfs-level file data.  We do that in the
		 * caller (__advfs_d_revalidate) by calling
		 * purge_inode_data.
		 */
		advfs_copy_attr_all(dentry->d_inode,
				      advfs_lower_inode(dentry->d_inode));
...
/*
 * Determine if the lower inode objects have changed from below the advfs
 ...
 * asynchronously.  This can sometimes cause advfs to find that the lower
 * file system doesn't change its inode times quick enough, resulting in a
 * false positive indication (which is harmless, it just makes advfs do
 ...
 * ADVFS_MIN_CC_TIME (which defaults to 3 seconds, as with NFS's acregmin)
 * because significant changes are more likely due to users manually
 * touching lower files.
 */
bool advfs_is_newer_lower(const struct dentry *dentry)
{
	int bindex;
	struct inode *inode;
	...
	if (!inode || !ADVFS_I(inode)->lower_inodes ||
	...
	for (bindex = ibstart(inode); bindex <= ibend(inode); bindex++) {
		...
		if (unlikely(timespec_compare(&inode->i_mtime,
			...
			if ((lower_inode->i_mtime.tv_sec -
			     inode->i_mtime.tv_sec) > ADVFS_MIN_CC_TIME) {
				pr_info("advfs: new lower inode mtime "
					"(bindex=%d, name=%s)\n", bindex,
					dentry->d_name.name);
		...
		if (unlikely(timespec_compare(&inode->i_ctime,
			...
			if ((lower_inode->i_ctime.tv_sec -
			     inode->i_ctime.tv_sec) > ADVFS_MIN_CC_TIME) {
				pr_info("advfs: new lower inode ctime "
					"(bindex=%d, name=%s)\n", bindex,
					dentry->d_name.name);
				show_dinode_times(dentry);
			}
	 ...
	 */
	if (advfs_is_negative_lower(dentry))
		return true;

	return false;		/* default: lower is not newer */
}

static int advfs_d_revalidate(struct dentry *dentry,
				struct nameidata *nd)
{
	bool valid = true;
...
static void advfs_d_release(struct dentry *dentry)
{
	advfs_read_lock(dentry->d_sb, ADVFS_SMUTEX_CHILD);
	if (unlikely(!ADVFS_D(dentry)))
 ...
 */
static void advfs_d_iput(struct dentry *dentry, struct inode *inode)
{
	int rc;
	...
	if (!ADVFS_D(dentry) || dbstart(dentry) < 0)
...
drop_lower_inodes:
	...
	if (rc == 1 && inode->i_nlink == 1 && ibstart(inode) >= 0) {
		...
		lockdep_off();
		iput(advfs_lower_inode(inode));
		lockdep_on();
		advfs_set_lower_inode(inode, NULL);
...
struct dentry_operations advfs_dops = {
	.d_revalidate	= advfs_d_revalidate,
	.d_release	= advfs_d_release,
	.d_iput		= advfs_d_iput,
};
