 ...
 * Documentation/filesystems/advfs/concepts.txt
 */
			 ...
			 struct dentry *new_lower_dentry)
{
	...
	name_list = advfs_xattr_alloc(list_size + 1, XATTR_LIST_MAX);
	...
	attr_value = advfs_xattr_alloc(XATTR_SIZE_MAX, XATTR_SIZE_MAX);
...
out:
	advfs_xattr_kfree(name_list_buf);
	advfs_xattr_kfree(attr_value);
			      ...
			      struct dentry *new_lower_dentry)
{
	...
	if (err == -EOPNOTSUPP &&
		...
		printk(KERN_WARNING
		       "advfs: changing \"%s\" symlink mode unsupported\n",
			    ...
			    char *symbuf)
{
	...
	if (S_ISDIR(old_mode)) {
		...
		advfs_run_sioq(__advfs_mkdir, &args);
		err = args.err;
		...
		advfs_run_sioq(__advfs_symlink, &args);
		err = args.err;
		...
		advfs_run_sioq(__advfs_mknod, &args);
		err = args.err;
		...
		advfs_run_sioq(__advfs_create, &args);
		err = args.err;
		advfs_release_lower_nd(&nd, err);
	} else {
		printk(KERN_CRIT "advfs: unknown inode type %d\n",
		       old_mode);
		BUG();
	}
...
static int __copyup_reg_data(struct dentry *dentry,
			     ...
			     struct file **advfs_copyup_file, loff_t len)
{
	...
	/* open old file */
	advfs_mntget(dentry, old_bindex);
	branchget(sb, old_bindex);
	/* dentry_open calls dput and mntput if it returns an error */
	input_file = dentry_open(old_lower_dentry,
				 advfs_lower_mnt_idx(dentry, old_bindex),
	...
	output_mnt = advfs_mntget(sb->s_root, new_bindex);
	...
	if (advfs_copyup_file) {
		*advfs_copyup_file = output_file;
		goto out_close_in;
	}
		    ...
		    struct dentry *new_lower_dentry, int new_bindex)
{
	...
	advfs_set_lower_dentry_idx(dentry, new_bindex, NULL);
 ...
 * @advfs_copyup_file: the "struct file" to return (optional)
 * @len: how many bytes to copy-up?
 */
int advfs_copyup_dentry(struct inode *dir, struct dentry *dentry, int bstart,
		  ...
		  struct file **advfs_copyup_file, loff_t len)
{
	...
	new_lower_dentry = advfs_create_parents(dir, dentry, name, new_bindex);
	if (IS_ERR(new_lower_dentry)) {
		err = PTR_ERR(new_lower_dentry);
		goto out;
	}

	old_lower_dentry = advfs_lower_dentry_idx(dentry, old_bindex);
	...
	if (err) {
		...
		if (dbstart(dentry) < old_bstart &&
			...
			__clear(dentry, NULL, old_bstart, old_bend,
				advfs_lower_dentry(dentry), dbstart(dentry));
		goto out;
	}
	if (!S_ISDIR(dentry->d_inode->i_mode)) {
		advfs_postcopyup_release(dentry);
		if (!advfs_lower_inode(dentry->d_inode)) {
			/*
			 * If we got here, then we copied up to an
			 * unlinked-open file, whose name is .advfsXXXXX.
			 */
			struct inode *inode = new_lower_dentry->d_inode;
			atomic_inc(&inode->i_count);
			advfs_set_lower_inode_idx(dentry->d_inode,
 ...
 * will be named "name".
 */
int advfs_copyup_named_file(struct inode *dir, struct file *file, char *name,
		      int bstart, int new_bindex, loff_t len)
{
	int err = 0;
	struct file *output_file = NULL;

	err = advfs_copyup_dentry(dir, file->f_path.dentry, bstart, new_bindex,
			    name, strlen(name), &output_file, len);
	if (!err) {
		fbstart(file) = new_bindex;
		advfs_set_lower_file_idx(file, new_bindex, output_file);
	}
 ...
 */
int advfs_copyup_file(struct inode *dir, struct file *file, int bstart,
		int new_bindex, loff_t len)
{
	...
	err = advfs_copyup_dentry(dir, dentry, bstart, new_bindex,
			     ...
			     int old_bstart, int old_bend)
{
	...
	for (i = loop_start; i <= loop_end; i++) {
		...
		if (!advfs_lower_dentry_idx(dentry, i)->d_inode) {
			dput(advfs_lower_dentry_idx(dentry, i));
			advfs_set_lower_dentry_idx(dentry, i, NULL);

			advfs_mntput(dentry, i);
			advfs_set_lower_mnt_idx(dentry, i, NULL);
		} else {
			if (new_bstart < 0)
				new_bstart = i;
			new_bend = i;
...
static void __set_inode(struct dentry *upper, struct dentry *lower,
			int bindex)
{
	advfs_set_lower_inode_idx(upper->d_inode, bindex,
				    igrab(lower->d_inode));
...
static void __set_dentry(struct dentry *upper, struct dentry *lower,
			 int bindex)
{
	advfs_set_lower_dentry_idx(upper, bindex, lower);
 ...
 */
struct dentry *advfs_create_parents(struct inode *dir, struct dentry *dentry,
			      const char *name, int bindex)
{
	...
	/* assume the negative dentry of advfs as the parent dentry */
	...
	do {
		...
		/* find the parent directory dentry in advfs */
		parent_dentry = dget_parent(child_dentry);

		/* find out the lower_parent_dentry in the given branch */
		lower_parent_dentry =
			advfs_lower_dentry_idx(parent_dentry, bindex);
...
begin:
	...
	lower_parent_dentry = advfs_lower_dentry_idx(parent_dentry, bindex);
	dput(parent_dentry);
	...
	if (child_dentry != dentry) {
		...
		advfs_set_lower_dentry_idx(dentry, bindex,
					     lower_dentry);
	...
	advfs_copy_attr_times(child_dentry->d_inode);
 ...
 */
void advfs_postcopyup_setmnt(struct dentry *dentry)
{
	...
	while (!advfs_lower_mnt_idx(hasone, bindex))
		hasone = hasone->d_parent;
	parent = dentry;
	while (!advfs_lower_mnt_idx(parent, bindex)) {
		advfs_set_lower_mnt_idx(parent, bindex,
					  advfs_mntget(hasone, bindex));
 ...
 */
void advfs_postcopyup_release(struct dentry *dentry)
{
	int bstart, bend;
