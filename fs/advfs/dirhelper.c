...
/* The callback structure for advfs_check_empty. */
struct advfs_rdutil_callback {
	int err;
	int filldir_called;
...
{
	int err = 0;
	struct advfs_rdutil_callback *buf = dirent;
	int is_whiteout;
...
int advfs_check_empty(struct dentry *dentry, struct dentry *parent,
		struct advfs_dir_state **namelist)
{
	...
	struct advfs_rdutil_callback *buf = NULL;
	...
	buf = kmalloc(sizeof(struct advfs_rdutil_callback), GFP_KERNEL);
	if (unlikely(!buf)) {
		err = -ENOMEM;
		goto out;
	}
	...
	buf->rdstate = advfs_alloc_rdstate(dentry->d_inode, bstart);
	...
	for (bindex = bstart; bindex <= bend; bindex++) {
		...
		dget(lower_dentry);
		mnt = advfs_mntget(dentry, bindex);
		branchget(sb, bindex);
	...
	if (buf) {
		if (namelist && !err)
			*namelist = buf->rdstate;
			...
			advfs_free_rdstate(buf->rdstate);
		kfree(buf);
	}
