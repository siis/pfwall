 ...
 * @dentry and @new_mnt.
 */
struct dentry *advfs___lookup_one(struct dentry *base, struct vfsmount *mnt,
...
/*
 * This is a utility function that fills in a advfs dentry.
 * Caller must lock this dentry with advfs_lock_dentry.
 *
 * Returns: 0 (ok), or -ERRNO if an error occurred.
 * XXX: get rid of _partial_lookup and make callers call _lookup_full directly
 */
int advfs_partial_lookup(struct dentry *dentry, struct dentry *parent)
{
	struct dentry *tmp;
	int err = -ENOSYS;

	tmp = advfs_lookup_full(dentry, parent, INTERPOSE_PARTIAL);
...
static struct kmem_cache *advfs_dentry_cachep;
int advfs_init_dentry_cache(void)
{
	advfs_dentry_cachep =
		kmem_cache_create("advfs_dentry",
				  sizeof(struct advfs_dentry_info),
				  0, SLAB_RECLAIM_ACCOUNT, NULL);

	return (advfs_dentry_cachep ? 0 : -ENOMEM);
}

void advfs_destroy_dentry_cache(void)
{
	if (advfs_dentry_cachep)
		kmem_cache_destroy(advfs_dentry_cachep);
}

void advfs_free_dentry_private_data(struct dentry *dentry)
{
	if (!dentry || !dentry->d_fsdata)
		return;
	kfree(ADVFS_D(dentry)->lower_paths);
	ADVFS_D(dentry)->lower_paths = NULL;
	kmem_cache_free(advfs_dentry_cachep, dentry->d_fsdata);
	dentry->d_fsdata = NULL;
}

static inline int __advfs_realloc_dentry_private_data(struct dentry *dentry)
{
	struct advfs_dentry_info *info = ADVFS_D(dentry);
	void *p;
	int size;
	...
	atomic_set(&info->generation,
			atomic_read(&ADVFS_SB(dentry->d_sb)->generation));
...
/* ADVFS_D(dentry)->lock must be locked */
int advfs_realloc_dentry_private_data(struct dentry *dentry)
{
	if (!__advfs_realloc_dentry_private_data(dentry))
		return 0;

	kfree(ADVFS_D(dentry)->lower_paths);
	advfs_free_dentry_private_data(dentry);
	return -ENOMEM;
}

/* allocate new dentry private data */
int advfs_new_dentry_private_data(struct dentry *dentry, int subclass)
{
	struct advfs_dentry_info *info = ADVFS_D(dentry);

	BUG_ON(info);

	info = kmem_cache_alloc(advfs_dentry_cachep, GFP_ATOMIC);
	if (unlikely(!info))
		return -ENOMEM;
 ...
 * starting branch
 */
void advfs_update_bstart(struct dentry *dentry)
{
	int bindex;
 ...
 * kfree the intent file (using advfs_release_lower_nd).
 *
 ...
 */
int advfs_init_lower_nd(struct nameidata *nd, unsigned int flags)
{
	int err = 0;
	...
	default:
		 ...
		 */
		pr_debug("advfs: unknown nameidata flag 0x%x\n", flags);
		BUG();
		break;
	}
...
void advfs_release_lower_nd(struct nameidata *nd, int err)
{
 ...
 * newly looked-up dentry), then advfs_lookup_backend will return a locked
 * dentry's info, which the caller must unlock.
 */
struct dentry *advfs_lookup_full(struct dentry *dentry,
				   struct dentry *parent, int lookupmode)
{
	 ...
	 * advfs_new_dentry_private_data already locked.
	 */
	...
	for (bindex = bstart; bindex <= bend; bindex++) {
		...
		lower_dir_dentry =
			advfs_lower_dentry_idx(parent, bindex);
		...
		wh_lower_dentry = advfs_lookup_whiteout(name, lower_dir_dentry);
		...
		lower_dir_mnt = advfs_lower_mnt_idx(parent, bindex);
		lower_mnt = NULL; /* XXX: needed? */

		lower_dentry = advfs___lookup_one(lower_dir_dentry, lower_dir_mnt,
					    name, &lower_mnt);
		...
		if (!lower_mnt)
			lower_mnt = advfs_mntget(dentry->d_sb->s_root,
						   bindex);
		 ...
		 * dbstart/dbend, even if the whole advfs dentry is
		...
		if (!S_ISDIR(lower_dentry->d_inode->i_mode))
			continue;
		opaque = advfs_is_opaque_dir(dentry, bindex);
		if (opaque < 0) {
			err = opaque;
			goto out_free;
		} else if (opaque) {
	...
	if (lookupmode == INTERPOSE_LOOKUP) {
		...
		if (advfs_lower_dentry_idx(dentry, bindex))
			goto out;
		lower_dir_dentry =
			advfs_lower_dentry_idx(parent, bindex);
		if (!lower_dir_dentry || !lower_dir_dentry->d_inode)
		...
		lower_mnt = advfs_mntget(dentry->d_sb->s_root, bindex);
		advfs_set_lower_mnt_idx(dentry, bindex, lower_mnt);
...
out_positive:
	...
	/*
	 * This advfs dentry is positive (at least one lower inode
	...
	pos_start = pos_end = -1;
	for (bindex = bstart; bindex <= bend; bindex++) {
		lower_dentry = advfs_lower_dentry_idx(dentry,
							bindex);
	 ...
	 * dentry.
	 */
	d_interposed = advfs_interpose(dentry, dentry->d_sb, lookupmode);
...
out:
	if (dentry && ADVFS_D(dentry)) {
		BUG_ON(dbstart(dentry) < 0 && dbend(dentry) >= 0);
		BUG_ON(dbstart(dentry) >= 0 && dbend(dentry) < 0);
	}
	if (d_interposed && ADVFS_D(d_interposed)) {
