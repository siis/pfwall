 ...
 * objects in advfs by deleting all possible lower inode objects from the
 ...
 *     as as per Documentation/filesystems/advfs/concepts.txt).
 *
 */
static int advfs_advfs_unlink_whiteout(struct inode *dir, struct dentry *dentry,
	...
	if (err) {
		if ((bindex == 0) ||
			...
			if (!IS_COPYUP_ERR(err))
				pr_debug("advfs: lower object deletion "
					     "failed in branch:%d\n", bindex);
			err = advfs_create_whiteout(dentry, sbstart(dentry->d_sb));
		}
	}
...
int advfs_unlink(struct inode *dir, struct dentry *dentry)
{
	int err = 0;
	...
	err = advfs_advfs_unlink_whiteout(dir, dentry, parent);
	/* call d_drop so the system "forgets" about us */
	if (!err) {
		advfs_postcopyup_release(dentry);
		advfs_postcopyup_setmnt(parent);
...
static int advfs_rmdir_first(struct inode *dir, struct dentry *dentry,
			       struct advfs_dir_state *namelist)
{
	...
	err = advfs_delete_whiteouts(dentry, dbstart(dentry), namelist);
	if (err)
		goto out;
	...
	set_nlink(dentry->d_inode, advfs_get_nlinks(dentry->d_inode));
...
int advfs_rmdir(struct inode *dir, struct dentry *dentry)
{
	int err = 0;
	...
	err = advfs_check_empty(dentry, parent, &namelist);
	if (err)
		goto out;

	err = advfs_rmdir_first(dir, dentry, namelist);
	...
	if (!err) {
		if (dstart < dend)
			err = advfs_create_whiteout(dentry, dstart);
	} else {
		int new_err;
		...
		new_err = advfs_create_whiteout(dentry, dstart - 1);
		if (new_err != -EEXIST)
			err = new_err;
	}
...
out:
	...
	if (!err) {
		...
		dput(advfs_lower_dentry_idx(dentry, dstart));
		advfs_set_lower_dentry_idx(dentry, dstart, NULL);
		d_drop(dentry);
